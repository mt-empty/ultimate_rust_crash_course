...
fn main() {
    ...
    let handle_a = thread::spawn(move || {
        println!("handle_a thread: Received {:?}", rx.recv());
    });

    pause_ms(100); // Make sure Thread A has time to get going before we spawn Thread B

    let handle_b = thread::spawn(move || {
        println!("handle_b thread: Received {:?}", rx2.recv());
    });

    for msg in 1..=4 {
        tx.send(format!("Main Thread: {}", msg)).unwrap();
    }

    drop(tx);
    handle_a.join().unwrap();
    handle_b.join().unwrap();
    println!("Main thread: Exiting Challenge.");
